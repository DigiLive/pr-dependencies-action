...
describe('Tag Extractor', () => {
  describe('Dependency Tags', () => {
    ...
    describe('Combined Formats', () => {
      ...
      `;
        ...
        expect(result).toContainEqual({ owner: 'owner', repo: 'repo', issue_number: 223 });
        expect(result).toContainEqual({ owner: 'org', repo: 'repo', issue_number: 256 });
      });
    });
  ...
  describe('Dependent Tags', () => {
    describe('getDependentsTags', () => {
      it('returns empty array when no dependents section exists', () => {
        expect(getDependentsTags('No dependents here')).toEqual([]);
      });

      it('extracts dependents from the section', () => {
